...
class TinymceConfigBuilder
{
     ...
     * @return array
     */
    public function build(array $config, array $bundles): array
    {
        $contentCss = [];
        foreach ($config['stylesheets']['contents'] as $path) {
            $contentCss[] = preg_match('~^/?bundles~', $path) ? 'asset[' . $path . ']' : $path;
        }
        $contentCss[] = 'asset[bundles/ekynaui/css/tinymce-content.css]';
        foreach ($config['stylesheets']['fonts'] as $path) {
            $contentCss[] = $path;
        }
        ...
        if (array_key_exists('front', $themes)) {
            $frontTheme = array_merge($frontTheme, $themes['front']);
        }
        ...
        $advancedTheme = [
            ...
            'content_css'               => $contentCss,
            'autoresize_max_height'     => '500',
            'images_upload_url'         => '/ui/tinymce/upload',
            //'images_upload_base_path' => '/some/basepath',
            'images_reuse_filename'     => true,
            'images_upload_credentials' => true,
        ];
        if (array_key_exists('advanced', $themes)) {
            $advancedTheme = array_merge($advancedTheme, $themes['advanced']);
        }

        // Color map
        if (!empty($config['colors'])) {
            $colorMap = [];
            foreach ($config['colors'] as $name => $hex) {
                $colorMap[] = $hex;
                $colorMap[] = $name;
            }
        ...
        // Styles formats
        switch ($config['tinymce']['base_formats']) {
            case 'default' :
        ...
        $customFormats = $config['tinymce']['custom_formats'];
        ...
        return [
            ...
            'tinymce_url' => '/bundles/ekynaui/lib/tinymce',
            'theme'       => $themes,
        ];
    }
     ...
     * @param array $b
     */
    private function mergeFormats(array &$a, array $b): void
    {
     ...
     * @return array
     */
    private function getBootstrapFormats(): array
    {
